...
class ClientProvider {
  ...
  Future<List<Client>> getAllDb(BuildContext context) async {
    final variable =
        await ClienteRepository.shared.selectAll(tableName: 'cliente');
  ...
  Future<void> initClientDb() async {
    ...
    ClienteRepository.shared
        .save(data: clientList.clientes, tableName: 'cliente');
  }
  ...
  Future<dynamic> logOutClientApi() async {
    await ApiManager.shared.request(
        baseUrl: "10.0.2.2:9595",
        pathUrl: "/cliente/logout",
        type: HttpType.GET);
  }

  Future<dynamic> logInDb(String email, String pass) async {
    return ClienteRepository.shared.selectWhere(
        tableName: 'cliente',
